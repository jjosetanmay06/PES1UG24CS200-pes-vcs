...
// ─── Phase 5 stubs (not required for Phases 1-4) ────────────────────────────
...
// ─── Phase 1-4 Command Implementations ──────────────────────────────────────

// pes init — create the .pes/ directory structure
void cmd_init(void) {
    mkdir(PES_DIR,     0755);
    ...
    if (!f) { perror("error: cannot write HEAD"); return; }
...
void cmd_add(int argc, char *argv[]) {
    ...
    // Use static to avoid ~5.6MB stack allocation (segfault fix)
    static Index index;
...
// Callback used by cmd_log to print each commit
