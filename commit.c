...
//   tree <64-hex>
//   parent <64-hex>      ← omitted for the first commit
...
//   2. Try to read current HEAD as the parent commit
//   3. Fill in a Commit struct and serialize it
//   4. Write the commit object and update HEAD
// Returns 0 on success, -1 on error.
int commit_create(const char *message, ObjectID *commit_id_out) {
    // Step 1: Build the tree snapshot from the staged index
    ObjectID tree_id;
    ...
    // Try to read the current HEAD as a parent (may not exist for first commit)
    ObjectID parent_id;
    ...
    // Step 3: Serialize the Commit struct to text
    void *raw;
    size_t raw_len;
    ...
    // Step 4a: Write the commit object to the object store
    ObjectID commit_id;
    ...
    // Step 4b: Atomically update the branch ref to the new commit
